...
public class WalkNodeProcessor extends NodeProcessor {
    ...
    public PathPoint getStart() {
        ...
        if (this.getCanSwim() && this.entity.inWater) {
            ...
            i = blockpos.getSide(Vector3.SIDE_UP).getFloorY();
        }
    ...
    public int findPathOptions(PathPoint[] pathOptions, PathPoint currentPoint, PathPoint targetPoint, float maxDistance) {
        ...
        Vector3 blockpos = (new Vector3(currentPoint.x, currentPoint.y, currentPoint.z)).getSide(Vector3.SIDE_DOWN);
        double d0 = (double) currentPoint.y - (1.0D - this.level.getBlock(blockpos).getBoundingBox().maxY);
     ...
     */
    private PathPoint getSafePoint(int x, int y, int z, int stepHeight, double diffY, EnumFacing facing) {
        PathPoint pathpoint = null;
        Vector3 pos1 = new Vector3(x, y, z);
        Vector3 pos2 = pos1.getSide(Vector3.SIDE_DOWN);
        double d0 = (double) y - (1.0D - this.level.getBlock(pos2).getBoundingBox().maxY);
        ...
        if (d0 - diffY > 1.125D) {
            ...
            if (pathnodetype == PathNodeType.WALKABLE) {
                ...
                if (pathpoint == null && stepHeight > 0 && pathnodetype != PathNodeType.FENCE && pathnodetype != PathNodeType.TRAPDOOR) {
                    pathpoint = this.getSafePoint(x, y + 1, z, stepHeight - 1, diffY, facing);
                    ...
                    if (pathpoint != null && (pathpoint.nodeType == PathNodeType.OPEN || pathpoint.nodeType == PathNodeType.WALKABLE) && this.entity.getWidth() < 1.0F) {
                        ...
                        AxisAlignedBB bb = new AxisAlignedBB(d2 - d1, (double) y + 0.001D, d3 - d1, d2 + d1, (double) ((float) y + this.entity.getHeight()), d3 + d1);
                        AxisAlignedBB bb2 = this.level.getBlock(pos1).getBoundingBox();
                        AxisAlignedBB bb3 = bb.addCoord(0.0D, bb2.maxY - 0.002D, 0.0D);

                        if (this.entity.getLevel().getCollisionBlocks(bb3, true).length != 0) {
                            pathpoint = null;
                        }
                    }
                }
        ...
        for (int i = 0; i < xSize; ++i) {
            for (int j = 0; j < ySize; ++j) {
                for (int k = 0; k < zSize; ++k) {
                    ...
                    if (pathnodetype1 == PathNodeType.RAIL && !(blockaccessIn.getBlock(blockpos) instanceof BlockRail) && !(blockaccessIn.getBlock(blockpos.getSide(Vector3.SIDE_DOWN)) instanceof BlockRail)) {
    ...
    public PathNodeType getPathNodeType(Level level, int x, int y, int z) {
        PathNodeType pathnodetype = this.getPathNodeTypeAt(level, x, y, z);

        if (pathnodetype == PathNodeType.OPEN && y >= 1) {
            PathNodeType pathnodetype1 = this.getPathNodeTypeAt(level, x, y - 1, z);
        ...
        if (pathnodetype == PathNodeType.WALKABLE) {
            for (int j = -1; j <= 1; ++j) {
                for (int i = -1; i <= 1; ++i) {
                    if (j != 0 || i != 0) {
                        int block1 = Utils.getBlockId(level, pos.setComponents(j + x, y, i + j));
    ...
    private PathNodeType getPathNodeTypeAt(Level level, int x, int y, int z) {
