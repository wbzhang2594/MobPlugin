...
public class PathNavigateGround extends PathNavigate {
    ...
    public Path getPathToPos(Vector3 pos) {
        if (this.level.getBlockIdAt(pos.getFloorX(), pos.getFloorY(), pos.getFloorZ()) == Block.AIR) {
            Vector3 blockpos = pos.getSide(BlockFace.DOWN);
            ...
            if (blockpos.getY() > 0) {
                return super.getPathToPos(blockpos.getSide(BlockFace.UP));
            }

            while (blockpos.getY() < 256 && Util.getBlockId(this.level, blockpos) == Block.AIR) {
                blockpos = blockpos.getSide(BlockFace.UP);
            }
        ...
        if (!this.level.getBlock(pos).isSolid()) {
            ...
            for (blockpos1 = pos.getSide(BlockFace.UP); blockpos1.getY() < 256 && Block.solid[Util.getBlockId(this.level, blockpos1)]; blockpos1 = blockpos1.getSide(BlockFace.UP)) {
                ;
            }
    ...
    private int getPathablePosY() {
        if (this.theEntity.inWater && this.getCanSwim()) {
            ...
            int block = Util.getBlockId(this.level, new Vector3(NukkitMath.floorDouble(this.theEntity.x), i, NukkitMath.floorDouble(this.theEntity.z)));
            int j = 0;

            while (block == Block.WATER || block == Block.STILL_WATER) {
                ++i;
                block = Util.getBlockId(this.level, new Vector3(NukkitMath.floorDouble(this.theEntity.x), i, NukkitMath.floorDouble(this.theEntity.z)));
                ++j;
    ...
    protected void removeSunnyPath() {
        ...
        for (int i = 0; i < this.currentPath.getCurrentPathLength(); ++i) {
            ...
            int block = Util.getBlockId(this.level, new Vector3(pathpoint.xCoord, pathpoint.yCoord, pathpoint.zCoord));
    ...
    private boolean isPositionClear(int p_179692_1_, int p_179692_2_, int p_179692_3_, int p_179692_4_, int p_179692_5_, int p_179692_6_, Vector3 p_179692_7_, double p_179692_8_, double p_179692_10_) {
        for (Vector3 blockpos : Util.getAllInBox(new Vector3(p_179692_1_, p_179692_2_, p_179692_3_), new Vector3(p_179692_1_ + p_179692_4_ - 1, p_179692_2_ + p_179692_5_ - 1, p_179692_3_ + p_179692_6_ - 1))) {
