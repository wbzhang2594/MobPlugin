...
public class Util {
    ...
    public static RayTraceResult rayTraceBlocks(Level level, Vector3 vec31, Vector3 vec32, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock) {
        ...
        while (k1-- >= 0) {
            ...
            /*if (d3 < d4 && d3 < d5) {
                face = i > l ? BlockFace.WEST : BlockFace.EAST;
                vec31 = new Vector3(d0, vec31.y + d7 * d3, vec31.z + d8 * d3);
            } else if (d4 < d5) {
                face = j > i1 ? BlockFace.DOWN : BlockFace.UP;
                vec31 = new Vector3(vec31.x + d6 * d4, d1, vec31.z + d8 * d4);
            } else {
                face = k > j1 ? BlockFace.NORTH : BlockFace.SOUTH;
                vec31 = new Vector3(vec31.x + d6 * d5, vec31.y + d7 * d5, d2);
            }

            l = NukkitMath.floorDouble(vec31.x) - (face == BlockFace.EAST ? 1 : 0);
            i1 = NukkitMath.floorDouble(vec31.y) - (face == BlockFace.UP ? 1 : 0);
            j1 = NukkitMath.floorDouble(vec31.z) - (face == BlockFace.SOUTH ? 1 : 0);*/
