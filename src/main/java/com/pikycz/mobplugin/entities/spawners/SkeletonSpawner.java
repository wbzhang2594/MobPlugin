...
public class SkeletonSpawner extends AbstractEntitySpawner {
    ...
    public SpawnResult spawn(IPlayer iPlayer, Position pos, Level level) {
        ...
        if (blockLightLevel > 7 || (level.getTime() < Level.TIME_NIGHT && !level.isThundering() && !level.isRaining() && level.canBlockSeeSky(pos))) { // lightlevel not working for now, but as lightlevel is always zero that should work
